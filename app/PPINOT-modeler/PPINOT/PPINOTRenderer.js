...
PPINOTRenderer.prototype.drawShape = function(p, element) {
    ...
    // Verificar si el elemento tiene un businessObject válido
    if (!element.businessObject) {
        console.warn('El elemento no tiene un businessObject válido:', element);
        return null; // Detener el proceso de renderizado si no hay businessObject
    }

    var h = this.renderers[type];

    // Establecer color predeterminado si no se ha definido
    if (!element.color) {
        element.color = "#000";
    }
